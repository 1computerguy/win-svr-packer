...
Configuration STANDALONE_ROOTCA
{
    ...
    Node $AllNodes.NodeName {
        ...
        # Set DNS so we can resolve Sub CAs
        ...
        WindowsFeature InstallWebMgmtConsole
        {
            Ensure    = 'Present'
        ...
        ADCSCertificationAuthority ConfigCA
        {
            ...
            KeyLength                 = 4096
            DependsOn                 = @('[File]CAPolicy','[xDisk]DiskE','[xDisk]DiskF')
        }
        ...
        xWebVirtualDirectory AddCertEnroll
        {
            Ensure          = 'Present'
            Name            = 'CertEnroll'
...
STANDALONE_ROOTCA -ConfigurationData .\StandaloneRootCA.psd1

Set-DscLocalConfigurationManager -Path .\STANDALONE_ROOTCA -Verbose -Force

Start-DscConfiguration -Wait -Path .\STANDALONE_ROOTCA -Verbose -Force